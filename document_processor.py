...
class DocumentProcessor:
    ...
    def run_tesseract(self, image: np.ndarray) -> str:
        ...
        try:
            pil_image = Image.fromarray(cv2.cvtColor(image, cv2.COLOR_RGB2RGB))
    ...
    def extract_text_from_image(self, img: np.ndarray) -> str:
        ...
        # If PaddleOCR fails, use Tesseract
    ...
    def process_image(self, file_path: str) -> Dict[str, Any]:
        ...
        if img is None:
            return {"error": f"Failed to load image: {file_path}", "file_type": "image", "file_name": os.path.basename(file_path)}
...
def main():
    ...
    # Create processing_result.json with the extracted text
    processing_result = {
    ...
    with open("processing_result.json", "w", encoding="utf-8") as f:
        json.dump(processing_result, f, indent=2, ensure_ascii=False)
    
    # Create important_details.json with structured information
    imp = proc.extract_important_details(res)
    with open("important_details.json", "w", encoding="utf-8") as f:
